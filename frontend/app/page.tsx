...
function FeatureCard({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all">
...
function AppMockup() {
  return (
    <div className="relative bg-gray-950 rounded-2xl overflow-hidden shadow-2xl border border-gray-800 max-w-2xl mx-auto">
      {/* top bar */}
      ...
      <div className="flex">
        ...
        <div className="w-44 bg-gray-900 border-r border-gray-800 p-3 space-y-1">
          {["⬛ Dashboard", "💬 Chat", "📝 Export", "📅 Termine"].map((item, i) => (
            <div key={i} className={`text-xs px-3 py-2 rounded-lg ${i === 1 ? "bg-indigo-600 text-white" : "text-gray-500"}`}>{item}</div>
          ))}
        </div>
        {/* content */}
        <div className="flex-1 p-4 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex justify-end">
              <div className="bg-indigo-600 text-white text-xs rounded-xl rounded-br-sm px-3 py-2 max-w-48">Was ist die Zeitkomplexität von Quicksort?</div>
            </div>
            <div className="flex">
            ...
            <div className="flex justify-end">
              <div className="bg-indigo-600 text-white text-xs rounded-xl rounded-br-sm px-3 py-2 max-w-48">Und im worst case?</div>
            </div>
            <div className="flex gap-1 pl-2">
              {[0, 1, 2].map(i => <div key={i} className="w-1.5 h-1.5 bg-gray-600 rounded-full animate-bounce" style={{ animationDelay: `${i * 150}ms` }} />)}
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="relative pt-24 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto text-center">
          <Badge>✨ Powered by Claude AI · Anthropic</Badge>
          ...
          <p className="mt-6 text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed">
            Lade deine PDFs, Skripte und Notizen hoch — und stelle Fragen direkt an dein Lernmaterial. KI antwortet basierend auf <strong className="text-gray-700">deiner</strong> Vorlesung, nicht Wikipedia.
          </p>

          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/app" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-8 py-4 rounded-2xl text-lg transition-colors shadow-lg shadow-indigo-200">
              Jetzt kostenlos starten →
            </Link>
            <a href="#how" className="bg-white hover:bg-gray-50 text-gray-700 font-semibold px-8 py-4 rounded-2xl text-lg border border-gray-200 transition-colors">
              Wie es funktioniert
            </a>
          </div>

          <p className="mt-4 text-sm text-gray-400">Kein Account nötig · Läuft lokal · Deine Daten bleiben deine Daten</p>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <PainCard emoji="😵" text="200-seitige Skripte — und du weißt nicht auf welcher Seite die Antwort steht." />
            <PainCard emoji="😰" text="Abgabetermine in 4 verschiedenen Apps, Zetteln und dem Kopf — und trotzdem vergisst man sie." />
            <PainCard emoji="😩" text="Mitschriften als unleserliche Textwand — kein Format, keine Struktur, nie auffindbar." />
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-20 px-6 bg-gray-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            <FeatureCard icon="💬" title="RAG Chat" desc="Stelle Fragen an deine eigenen PDFs und Notizen. Die KI antwortet mit Inhalt aus deiner Vorlesung — nicht aus dem Internet." />
            <FeatureCard icon="📝" title="Word Export" desc="Notizen in 3 professionellen Designs als .docx herunterladen — Minimal, Akademisch oder Modern." />
            <FeatureCard icon="📅" title="Abgabetermine" desc="Deadlines mit Fach, Datum und Priorität eintragen. Countdown zeigt dir wie viel Zeit noch bleibt." />
            <FeatureCard icon="📊" title="Lerntracker" desc="Lernstreak, Aktivitätsdiagramm und Statistiken — sieh wie konsequent du wirklich lernst." />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="space-y-7">
              <Step n="1" title="Dokument hochladen" desc="Lade dein Vorlesungsskript, deine Mitschriften oder ein Word-Dokument hoch. PDF, DOCX, TXT und Markdown werden unterstützt." />
              <Step n="2" title="Frage stellen" desc='Tippe deine Frage auf Deutsch — z.B. "Erkläre mir den Unterschied zwischen Stack und Heap" — und erhalte eine Antwort basierend auf deiner Vorlesung.' />
              <Step n="3" title="Exportieren & Tracken" desc="Speichere deine Notizen als professionelles Word-Dokument und behalte deine Abgabetermine im Dashboard im Blick." />
            </div>
          </div>
      ...
      {/* CTA */}
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto bg-gradient-to-br from-indigo-600 to-violet-600 rounded-3xl p-12 text-center text-white shadow-2xl shadow-indigo-200">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Bereit schlauer zu lernen?</h2>
          <p className="text-indigo-200 text-lg mb-8 max-w-xl mx-auto">Lade dein erstes Vorlesungsskript hoch und stell deine erste Frage — in unter 2 Minuten.</p>
          ...
          <p className="text-indigo-300 text-sm mt-4">Kein Account · Keine Kreditkarte · Sofort loslegen</p>
        </div>
      </section>
      ...
      <footer className="border-t border-gray-100 py-10 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-gray-400 text-sm">Gebaut von <a href="https://giorgiodettmar.com" className="text-indigo-600 hover:underline">Giorgio Dettmar</a> · Powered by Claude AI</p>
